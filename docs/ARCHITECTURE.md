...
- `context` — only present with the `c` flag (so a separator inside the text can't be mistaken for it): a few lines about the game, character, zone, map coordinates, money, talents, professions and quest log ids (`WoWClaude.GameContext()`, capped at 900 bytes; since coordinates change as you move, most messages sent after walking somewhere carry a fresh copy). Every hello carries it (empty when `/wow-claude context off`); a message carries it only when it differs from the last version the bridge acknowledged, and only if it fits next to the text.
